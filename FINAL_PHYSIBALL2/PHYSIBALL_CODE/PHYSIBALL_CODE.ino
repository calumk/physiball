...
char readChar()
{
  while (mySerial.available() < 1) { } // Block
...
void loop()
{
  ...
  {
    if (mySerial.read() == '#') // Start of new control message
    {
      int command = mySerial.read(); // Commands
      ...
      else if (command == 'o') // Set _o_utput mode
      {
        ...
        else if (output_param == 'e') // _e_rror output settings
        {
          ...
          {
            mySerial.print("#AMG-ERR:");
            mySerial.print(num_accel_errors); mySerial.print(",");
            mySerial.print(num_magn_errors); mySerial.print(",");
            mySerial.println(num_gyro_errors);
          }
        }
      }
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    mySerial.print("loop time (ms) = ");
    mySerial.println(millis() - timestamp);
#endif
  }
#if DEBUG__PRINT_LOOP_TIME == true
  else
